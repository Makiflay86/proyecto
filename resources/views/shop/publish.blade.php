            ...
            <form action="{{ route('publish.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                <div class="field-group">
                    ...
                    <textarea id="descripcion" name="descripcion" rows="4"
                              @class(['w-full rounded-xl border bg-gray-50 dark:bg-gray-700 px-4 py-2.5 text-sm text-gray-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-gold-400 transition', 'border-red-500' => $errors->has('descripcion'), 'border-gray-300 dark:border-gray-600' => !$errors->has('descripcion')])>{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <p class="field-error text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
